...
struct Day23 {
    ...
    static func part2(_ input: String) -> String {
        ...
        func largestGroups(frontier: [[String]]) -> [[String]] {
            
            let nextFrontier: [[String]] = frontier.flatMap { group in
                
                computers.filter { computer in
                    let neighbors = connectedComputers[computer]!
                    return Set(neighbors).intersection(Set(group)).count == group.count
                }
                .map { computer in
                    group + [computer]
                }
            ...
            if nextFrontier.isEmpty { return frontier }

            let uniqueNextFrontier = Set(nextFrontier.map { group in group.sorted().joined(separator: "-") })
                .map { groupString in groupString.split(separator: "-").map { String($0) }}
            
            return largestGroups(frontier: uniqueNextFrontier)
        }
                
        let largestGroup = largestGroups(frontier: computers.map { [$0] }).first!
        
        let password = largestGroup.joined(separator: ",")
